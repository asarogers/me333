...
#include "utilities.h"
#include "ina219.h"
#define MAX_TRAJECTORY_SAMPLES 1000  // Adjust as needed
...
static float actualAngleArray[MAX_TRAJECTORY_SAMPLES];   // Added declaration
static float referenceAngleArray[MAX_TRAJECTORY_SAMPLES]; 
...
void __ISR(_TIMER_2_VECTOR, IPL5SOFT) Controller(void) {
    ...
    switch (get_mode()) {
        ...
        case TRACK: {
            actualCurrent = -INA219_read_current();
            float track_error = refCurrentFromPosition - actualCurrent;
        
            integral += track_error;
            pwmDuty = ((kp_mA * track_error) + (ki_mA * integral));
...
void __ISR(_TIMER_1_VECTOR, IPL6SOFT) PositionController(void)
{
    ...
    //     actualAngleArray[trajectory_index] = actualAngle;
    //     referenceAngleArray[trajectory_index] = desired_deg;
    // }
    ...
    while (1) {
        ...
        switch (buffer[0]) {
            ...
            case 'm': { // Load step trajectory
                NU32DIP_ReadUART1(buffer, BUF_SIZE);
                sscanf(buffer, "%d", &trajectory_length);
            
                if (trajectory_length > MAX_TRAJECTORY_SAMPLES) {
                ...
                for (int i = 0; i < trajectory_length; i++) {
                    NU32DIP_ReadUART1(buffer, BUF_SIZE);
                    sscanf(buffer, "%f", &trajectory[i]);  // Store as degrees
                }
            
                NU32DIP_WriteUART1("Step trajectory loaded.\r\n");
                break;
            }
            
            case 'n': { // Load cubic trajectory#include "nu32dip.h" // config bits, constants, funcs for startup and UART
#include "encoder.h"
#include "utilities.h"
#include "ina219.h"
#define MAX_TRAJECTORY_SAMPLES 1000  // Adjust as needed
    ...
    while (1) {
        ...
        switch (buffer[0]) {
            ...
            case 'n': { // Load cubic trajectory
                ...
                NU32DIP_WriteUART1("Cubic trajectory loaded.\r\n");
                break;
            }
...
void sendDataToPython(int *refArray, int *actualArray, int length) {
    ...
    for (int i = 0; i < length; i++) {
        ...
        NU32DIP_WriteUART1(buffer);
            ...
            case 's': { // Send trajectory data
                char buffer[50];
                sprintf(buffer, "%d\n", trajectory_length);
                NU32DIP_WriteUART1(buffer);
            
                for (int i = 0; i < trajectory_length; i++) {
                    sprintf(buffer, "%f %f\n", referenceAngleArray[i], actualAngleArray[i]);
                    NU32DIP_WriteUART1(buffer);
                }
                break;
            }
